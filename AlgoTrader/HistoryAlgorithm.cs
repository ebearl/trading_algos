...
namespace QuantConnect.Algorithm.CSharp
{
    ...
    public class test : QCAlgorithm, IRegressionAlgorithmDefinition
    {
        ...
        public override void Initialize()
        {
            ...
            var assetPrice = new Series("Asset Price", SeriesType.Scatter, "$", Color.Red, ScatterMarkerSymbol.Triangle);
            ...
            foreach (decimal low in quandlSpyLows)
            {
            	if(lowLoopCount < 7) {
            		...
            		var flagSeries = new Series($"LowP {lowLoopCount}", SeriesType.Flag, "$", Color.Red, ScatterMarkerSymbol.Triangle);
            		stockPlot.AddSeries(flagSeries);
            		Debug("add new Flag series");
            	}
        ...
        {
            _count++;
            Plot("Trade Plot", "LowP {lowLoopCount}", data.Bars["SPY"].Close);
